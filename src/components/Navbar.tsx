...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full border-b border-border/40 bg-background/80 backdrop-blur-md supports-[backdrop-filter]:bg-background/60 shadow-sm py-4 transition-all duration-300">
        ...
        <div className="hidden md:flex items-center gap-6">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
              className={`relative text-sm font-medium transition-[color,transform] duration-300 ease-[cubic-bezier(0.21,0.47,0.32,0.98)] motion-safe:hover:text-primary motion-safe:hover:-translate-y-px group ${
      ...
      {/* Mobile Nav Menu */}
      <div className={`fixed inset-0 top-[73px] z-40 bg-background/95 backdrop-blur-xl transition-all duration-300 md:hidden ${isMobileMenuOpen ? "opacity-100 visible" : "opacity-0 invisible"}`}>
        <div className="flex flex-col items-center justify-center h-full space-y-6 pb-20">
          {navLinks.map((link) => (
            <Link
              ...
              className={`block text-2xl font-serif font-medium py-2 transition-[color,transform,opacity] duration-300 ease-[cubic-bezier(0.21,0.47,0.32,0.98)] motion-safe:active:scale-[0.99] ${
                location === link.href ? "text-primary" : "text-muted-foreground"
              }`}
            >
              {link.label}
